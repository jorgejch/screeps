...
module.exports = hr = {
    ...
    setRoles: function (num_hostiles) {
        const print_stats = function (state) {
            console.log(`${state} | guards ${Memory.guards.length}/${Memory.numGuards} | repairmans ${Memory.repairmans.length}/${Memory.numRepairmans} | upgraders  ${Memory.upgraders.length}/${Memory.numUpgraders} | builders   ${Memory.builders.length}/${Memory.numBuilders} | harvesters ${Memory.harvesters.length}/${Memory.numHarvesters} |`);
        };

        if (num_hostiles) {
            hr.personelConfiguration.war(num_hostiles);
            print_stats("war");
        }
        else {
